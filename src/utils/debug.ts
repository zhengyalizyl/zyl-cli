...
const debugError = (msg: string) => {
  debugSwitch && log(chalk.hex('#646cff')(`[zyl-cli]:`) + chalk.red(msg));
  // 如果出错就退出
  process.exit(0);
};
...
const debugInfo = (msg: string) => {
  debugSwitch && log(chalk.hex('#646cff')(`[zyl-cli]:`) + chalk.green(msg));
};
...
const debugProcess = (msg: string) => {
  debugSwitch && log(chalk.hex('#646cff')(`[zyl-cli]:`) + chalk.yellow(msg));
};
/**
 * debug warning信息
 * @param type 类型
...
const debugWarning = (msg: string) => {
  log(chalk.hex('#646cff')(`[zyl-cli]:`) + chalk.yellow(msg));
};

const debugTxt = (msg: string) => {
  log(chalk.hex('#646cff')(`[zyl-cli]:`) + chalk.hex('#5c6d82')(msg));
};
